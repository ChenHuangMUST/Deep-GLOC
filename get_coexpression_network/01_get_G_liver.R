# =========================================================
# Script: 01_get_G_liver.R
# Brief:
#   Define the liver-expressed gene universe (G_liver) from
#   GTEx liver transcriptomic data for Deep-GLOC.
#
# Input:
#   GTEx liver expression data and gene annotation files.
#
# Output:
#   G_liver gene set and processed expression matrices for
#   downstream network construction.
#
# Note:
#   Run this script before 02_build_meta_graph.R.
# =========================================================
...
infile <- get_arg("--input", Sys.getenv("DEEPGLOC_GTEX_GCT", ""))
outdir <- get_arg("--outdir", file.path(script_dir, "outputs", "G_liver"))

if (infile == "") {
  stop(
    "Please provide the GTEx liver GCT file with --input=<path> ",
    "or set DEEPGLOC_GTEX_GCT."
  )
}
...
save.image(file.path(outdir, "code_Gliver.RData"))
