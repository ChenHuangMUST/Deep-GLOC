# =========================================================
# Script: 02_build_meta_graph.R
# Brief:
#   Construct the liver meta co-expression graph for
#   Deep-GLOC by integrating multiple NAFLD bulk
#   transcriptomic datasets within the G_liver background.
#
# Input:
#   G_liver generated by 01_get_G_liver.R and harmonized
#   bulk transcriptomic datasets.
#
# Output:
#   Cohort-specific co-expression networks and the
#   integrated meta graph for downstream augmented graph
#   construction.
#
# Note:
#   Run this script after 01_get_G_liver.R.
# =========================================================
...
G_liver_file <- get_arg("--g-liver-file", Sys.getenv("DEEPGLOC_G_LIVER_FILE", ""))
expr_dir <- get_arg("--expr-dir", Sys.getenv("DEEPGLOC_EXPR_DIR", ""))
core_gene_file <- get_arg("--core-gene-file", Sys.getenv("DEEPGLOC_CORE_GENE_FILE", ""))
outdir <- get_arg("--outdir", file.path(script_dir, "outputs", "meta_graph"))
datasets_arg <- get_arg("--datasets", "")

if (G_liver_file == "") {
  stop("Please provide --g-liver-file=<path> or set DEEPGLOC_G_LIVER_FILE.")
}
if (expr_dir == "") {
  stop("Please provide --expr-dir=<path> or set DEEPGLOC_EXPR_DIR.")
}
...
read_logCPM <- function(gse){
  file <- file.path(expr_dir, paste0(gse, "_logCPM.txt"))
...
datasets <- if (datasets_arg == "") {
  c("GSE126848","GSE167523","GSE174478",
    "GSE193066","GSE193080","GSE225740")
} else {
  trimws(strsplit(datasets_arg, ",")[[1]])
}
###批量读取并过滤
...
write.table(
  edge_df,
  file = file.path(outdir, "consensus_network_edge_list.txt"),
  sep = "\t", quote = FALSE, row.names = FALSE
)
...
if (core_gene_file != "" && file.exists(core_gene_file)) {
  core_genes <- read.csv(core_gene_file)
  print(intersect(bb, core_genes$gene_id))
  print(setdiff(core_genes$gene_id, bb))
  print(core_genes[core_genes$gene_id %in% bb, ])
}


save.image(file.path(outdir, "code_meta_graph_new.RData"))
